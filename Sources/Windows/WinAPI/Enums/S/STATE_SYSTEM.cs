/**************************************************************************************************************
    ...
    NAME
        WinAPI/Constants/S/STATE_SYSTEM.cs
...
namespace Thrak. WinAPI. Enums
   {
	# region STATE_SYSTEM_ object state constants.
	/// <summary>
	/// An object is associated with one or more of these state values at any time. The following object state constants are not used : STATE_SYSTEM_ALERT_HIGH, 
	/// STATE_SYSTEM_ALERT_MEDIUM, STATE_SYSTEM_ALERT_LOW, and STATE_SYSTEM_FLOATING.
	/// <para>
	/// Clients retrieve an object's state by calling IAccessible::get_accState, which returns an integer that is a combination of the following bit flags. 
	/// </para>
	/// <para>
	/// Clients call GetStateText with the state value to retrieve a localized string that describes the object's state.
	/// </para>
	/// <para>
	/// When the state of an object changes, servers should call NotifyWinEvent with the EVENT_OBJECT_STATECHANGE event constant. 
	/// However, objects with the STATE_SYSTEM_INVISIBLE, STATE_SYSTEM_FOCUSED, and STATE_SYSTEM_ SELECTED object state constants have their own event constants. 
	/// For these objects, do not use EVENT_OBJECT_STATECHANGE. Instead, use the individual event constant. 
	/// </para>
	/// </summary>
	public enum  STATE_SYSTEM_Constants	: uint
	   {
		/// <summary>
		/// Zero value.
		/// </summary>
		STATE_SYSTEM_NONE			=  0x00000000,

		/// <summary>
		/// (This object state constant is not supported.) 
		/// <para>
		/// Indicates important information to be immediately conveyed to the user. For example, when a battery reaches a critically low level, 
		/// a level indicator generates a high-level alert. As a result, a blind access tool immediately announces this information to the user, 
		...
		/// (This object state constant is not supported.)
		/// <para>
		/// Indicates low-priority information that is not important to the user. This state is used, for example, when Word changes the appearance of 
		/// the TipWizard button on its toolbar to indicate that it has a hint for the user.
		/// </para>
		/// </summary>
		STATE_SYSTEM_ALERT_LOW       		=  0x08000000,

		/// <summary>
		/// (This object state constant is not supported.)
		/// <para>
		/// Indicates important information that is not conveyed immediately to the user. For example, when a battery is starting to reach a low level, 
		/// a level indicator generates a medium-level alert. A blind access tool then generates a sound to let the user know that important information 
		/// is available, without actually interrupting the user's work. The user could then query the alert information when convenient.
		/// </para>
		/// </summary>
		STATE_SYSTEM_ALERT_MEDIUM          	=  0x04000000,

		/// <summary>
		/// The object's appearance changes rapidly or constantly. Graphics that are animated occasionally are described as ROLE_SYSTEM_GRAPHIC 
		/// with the State property set to STATE_SYSTEM_ANIMATED. This state is used to indicate that the object's location is changing.
		/// </summary>
		...
		/// <summary>
		/// The object is programmatically hidden. For example, menu itmes are programmatically hidden until a user activates the menu. 
		/// <para>
		...
		/// </para>
		/// <para>
		/// However, if client applications find an object with this state, they should check whether STATE_SYSTEM_OFFSCREEN is also set. 
		/// </para>
		/// <para>
		/// If this second state is defined, clients can communicate the information about the object to users. For example, a list box can have both 
		/// STATE_SYSTEM_INVISIBLE and STATE_SYSTEM_OFFSCREEN set. In this case, the client application can communicate all items in the list to users. 
		/// If a client application is navigating through an IAccessible tree and encounters a parent object that is invisible, Microsoft Active Accessibility 
		/// will not expose information about any possible children of the parent as long as the parent is invisible.
		/// </para>
		/// </summary>
		...
		/// </para>
		/// <para>
		/// Same as STATE_SYSTEM_INDETERMINATE.
		/// </para>
		/// </summary>
		...
		/// <summary>
		/// A mask for determinating if the object is valid.
		/// </summary>
